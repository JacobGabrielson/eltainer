...
production code never shells out for container lifecycle."
  (let* ((cfg (docker-config-detect))
         (name (format "eltainer-logs-test-%d" (random 100000))))
    (unless (zerop (call-process "docker" nil nil nil "run" "-d" "--name" name
                                 "alpine:3.20" "sh" "-c"
                                 "echo eltainer-test-marker && sleep 30"))
          ...
          (with-current-buffer buf
            (let ((deadline (+ (float-time) 5.0)))
              (while (and (< (float-time) deadline)
                          (not (string-match-p "eltainer-test-marker"
                                               (buffer-string))))
                (accept-process-output docker-logs--process 0.1)))
            (should (string-match-p "eltainer-test-marker" (buffer-string))))
          (kill-buffer buf))
